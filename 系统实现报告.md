...
前端可以通过`pnpm install`安装依赖，然后通过`pnpm dev`运行。

### 后端
需要安装 Python 和 Django。

后端包含如下依赖。`pip install`安装下列依赖后，运行`manage.py runserver`执行。
```
django
corsheaders
django-cors-headers
...
### 3.1.1 总览
基本表如下图。
![tables.drawio](.\pictures\tables.drawio.svg)
