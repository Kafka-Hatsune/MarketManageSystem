...
![login (1)](D:\AI\DB\Documentation\pictures\运行示例\login (1).png)

用户名密码的限制

![login (1)](D:\AI\DB\Documentation\pictures\运行示例\login (2).png)

错误的账号密码

![login (1)](D:\AI\DB\Documentation\pictures\运行示例\login (3).png)

### 7.1.2 注册

![login (1)](D:\AI\DB\Documentation\pictures\运行示例\login (4).png)

![login (1)](D:\AI\DB\Documentation\pictures\运行示例\login (5).png)
...
**@王静：**数据库的大作业可以算是我的第一个项目，完全从零开始学习后端开发，倾注了许多心血，但同时也有很多方面的收获。首先，在数据库设计和管理的过程中加深了我对课堂学习的理论知识的理解，在逐步修改完善表格的过程中体会怎样让关系模式更加规范。其次在后端的开发技术上更是取得了飞跃性的进步，记得十一月中旬刚开始写后端的时候，被身份验证和前后端传图片的问题困扰了好几天，经过第一次联调之后，逐渐熟练起来，写接口更多地变成了体力活，后期已经开始考虑实现excel批量导入数据等高级功能。实现了后端所有功能之后特意数了一下，一共恰好写了五十个接口，还是很有成就感的。此外，来自队友的技术指导以及共同讨论功能与模型的经历也是这次作业体验中珍贵的一部分。总之，这个项目确实让我学习了很多，是一次很宝贵的经历。

**@钟彦童：**从一开始的考虑我们想要一个什么样的系统，再到根据需求讨论每个模块的功能，我对数据库的设计有了更深入的了解，对数据库的设计有了更深入的认识。总结、整理、分析我们的系统架构和设计的过程中，我深深地体会到了数据库设计过程的重要性。从需求分析，到概念模式的设计，再到逻辑模式，每一个步骤都需要细心和耐心。从宏观到微观地梳理整个系统的功能，再投入到测试环节，让我对我们的系统有了更深入的理解。我相信这次的经历对我未来的学习有着深远的影响。